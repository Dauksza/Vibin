...
OUT_PATHS = [Path('/mnt/agents/output/forge'), Path('./forge')]

def write_all(out: Path):
    out.mkdir(parents=True, exist_ok=True)
    (out / "forge").mkdir(parents=True, exist_ok=True)
    (out / "forge" / "static").mkdir(parents=True, exist_ok=True)
...
""")
    ...
    # README.md
    (out / "README.md").write_text("""# Forge
...
## Running with Real Ollama Models

```bash
ollama pull qwen3-coder
ollama pull deepseek-r1:14b
forge run "add vibration threshold alerting to machines API"
```
""")

    # forge/__init__.py
    (out / "forge" / "__init__.py").write_text('__version__ = "0.1.0"\n')

    # forge/events.py
    (out / "forge" / "events.py").write_text('''"""Event bus: single source of truth for execution telemetry and UI streams."""
...
''')
    ...
    # forge/dag.py
    (out / "forge" / "dag.py").write_text('''"""Deterministic task graph execution: lanes in parallel, deps enforced by the system."""
from __future__ import annotations
import threading
...
''')
    ...
    # forge/realms.py
    (out / "forge" / "realms.py").write_text('''"""Realm: an isolatable, snapshot-able part of the workspace. File realm is git-backed."""
from __future__ import annotations
...
''')
    ...
    # forge/worktree.py
    (out / "forge" / "worktree.py").write_text('''"""Parallel lanes: each lane gets a git worktree; merges happen behind verification gates."""
from __future__ import annotations
from pathlib import Path
...
''')
    ...
    # forge/manifest.py
    (out / "forge" / "manifest.py").write_text('''"""forge.json — the project manifest. The system reads it; models only propose."""
import json
from pathlib import Path
...
''')
    ...
    # forge/evidence.py
    (out / "forge" / "evidence.py").write_text('''"""Evidence bundles: every diagnose/judge call consumes concrete artifacts, not vibes."""
from __future__ import annotations
import shutil
from pathlib import Path
...
''')
    ...
    # forge/ollama.py
    (out / "forge" / "ollama.py").write_text('''"""Ollama API client: auto-discovery, models list, and streaming-free chat calls."""
from __future__ import annotations
...
''')
    ...
    # forge/router.py
    (out / "forge" / "router.py").write_text('''"""Capability-based model routing with first-class fallback chains."""
from __future__ import annotations
...
def _score(model, kind: str) -> float:
    ...
    # Crucial rule verified by test: coder models cannot judge their own output
...
''')
    ...
    # forge/knowledge.py
    (out / "forge" / "knowledge.py").write_text('''"""Project intelligence: symbol graph + inverted index for semantic grounding."""
from __future__ import annotations
import re
from pathlib import Path
...
''')
    ...
    # forge/contracts.py
    (out / "forge" / "contracts.py").write_text('''"""The 6-layer verification contract stack. System owns verdicts; models never grade themselves."""
from __future__ import annotations
...
''')
    ...
    # forge/agents.py
    (out / "forge" / "agents.py").write_text('''"""Agents propose; system disposes. Implements OllamaAgent and scripted FakeAgent."""
from __future__ import annotations
import json
...
''')
    ...
    (out / "forge" / "statemachine.py").write_text('''"""The deterministic control loop:
...
''')
    ...
    # forge/demo.py
    (out / "forge" / "demo.py").write_text('''"""Offline end-to-end demo of PlantWatch scenario:
...
# The injected regression: login inherits conflicting dashboard cards CSS grid and leaks sidebar styles
...
''')
    ...
    # forge/cli.py
    (out / "forge" / "cli.py").write_text('''"""forge CLI — control plane entry point."""
from __future__ import annotations
...
''')
    ...
    # forge/webui.py
    (out / "forge" / "webui.py").write_text('''"""Local IDE server for Forge."""
from __future__ import annotations
...
''')
    ...
    # tests/test_forge.py
    (out / "tests" / "test_forge.py").write_text('''import json, tempfile, unittest
from pathlib import Path
from forge.dag import DAG, Task, PlanError
from forge.router import Router, _score
from forge.realms import FileRealm, git
from forge.demo import run_demo, scaffold, record_visual_baseline, LOGIN_GOOD, DASH_NEW
...
for p in OUT_PATHS:
    write_all(p)
print("Forge generated successfully in", [str(p) for p in OUT_PATHS])
